export const AccountPlaceholder = '최대 8글자의 결제수단명을 입력해주세요.';
export const CategoryPlaceholder = '최대 8글자의 카테고리명을 입력해주세요.';
export const CreateAccountModalName = '결제수단 생성';
export const CreateIncomeCategoryModalName = '수입 카테고리 생성';
export const CreateExpenditureCategoryModalName = '지출 카테고리 생성';
export const UpdateIncomeCategoryModalName = '수입 카테고리 편집';
export const UpdateExpenditureCategoryModalName = '지출 카테고리 편집';
export const AccountLabelName = '결제수단 이름';
export const CategoryLabelName = '카테고리 이름';
export const UpdateAccountModalName = '결제수단 편집';
export const UpdateCategorytModalName = '카테고리 편집';

const formModal = {
  AccountPlaceholder,
  CategoryPlaceholder,
  CreateAccountModalName,
  CreateIncomeCategoryModalName,
  CreateExpenditureCategoryModalName,
  UpdateIncomeCategoryModalName,
  UpdateExpenditureCategoryModalName,
  AccountLabelName,
  CategoryLabelName,
  UpdateAccountModalName,
  UpdateCategorytModalName,
};

export default formModal;
